...
- Hiding the chrome (paper-only mode) in the Simulator is a **two-finger tap = Option+click** (Simulator-only gesture); a second one brings it back. Single taps start strokes under `.anyInput` and never reach the `TapGesture` that toggles the UI on device.
